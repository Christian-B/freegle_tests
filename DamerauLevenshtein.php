 ...
 *
 * Sourced from https://github.com/Oefenweb/damerau-levenshtein
 * All mb_ functions converted to simple String ones
 */
class DamerauLevenshtein
{
    ...
    {
        #echo " getSimilarity()<br>";
        if (!$this->calculated) {
            $this->setupMatrix();
        }
        #var_dump($this->matrix);
        return $this->matrix[strlen($this->compOne)][strlen($this->compTwo)];
    }
    ...
    private function setupMatrix()
    {
        ...
        $oneSize = strlen($this->compOne);
        $twoSize = strlen($this->compTwo);
        ...
        for ($i = 1; $i <= $oneSize; $i += 1) {
            ...
            $cOne = substr($this->compOne, $i - 1, 1);
            for ($j = 1; $j <= $twoSize; $j += 1) {
                // Curchar for the second string
                $cTwo = substr($this->compTwo, $j - 1, 1);
                ...
                if (($i > 1) && ($j > 1)) {
                    // Last two
                    $ccOne = substr($this->compOne, $i - 2, 1);
                    $ccTwo = substr($this->compTwo, $j - 2, 1);
